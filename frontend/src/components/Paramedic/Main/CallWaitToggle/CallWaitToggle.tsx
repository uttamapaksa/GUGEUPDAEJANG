...
function CallWaitToggle() {
  ...
  const clickButton = (num: number) => {
    if (selected === num) return setSelected(0);
  ...
  return (
    <S.Container>
      <A.BtnParaState onClick={() => (clickButton(1), goToCall())} $IsClick={isSelected(1)}>
        ...
        <S.ImgDiv>{selected === 2 ? <A.ImgAmbulance $width="4vh" /> : <A.ImgAmbulanceActive $width="4vh" />}</S.ImgDiv>
      </A.BtnParaState>
      ...
      <A.BtnParaState onClick={() => clickButton(2)} $IsClick={isSelected(2)}>
        ...
        <S.ImgDiv>
          {selected === 2 ? <A.ImgRequestBellActive $width="3.5vh" /> : <A.ImgRequestBell $width="3.5vh" />}
        </S.ImgDiv>
      </A.BtnParaState>
    </S.Container>
  );
