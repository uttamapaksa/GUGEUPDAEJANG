...
export const BtnSubmit = styled(StyledButton)`
  ...
  font-size: ${(props) => (props.$fontSize ? `${props.$fontSize}` : `${theme.font.Medium3_23};`)};
  font-weight: 600;
`;
...
export const BtnKtas = styled(StyledButton)`
  ...
  color: ${(props) =>
    props.$ktas && ['3', '5'].includes(props.$ktas?.slice(4))
      ? props.$IsClick
        ? `${theme.color.black}`
        : `${theme.color.fontGrey1}`
      : `${theme.color.white}`};
  background-color: ${(props) =>
    props.$ktas && props.$IsClick
      ? theme.color[`${props.$ktas}_Active`]
      : props.$ktas && !props.$IsClick
      ? theme.color[`${props.$ktas}_Deactive`]
      : ''};
  transition: color 0.2s ease-in-out, background-color 0.2s ease-in-out;
`;
